...
//Unity MonoBehaviour를 위한 제너릭 싱글톤 구현
...
{ 
   ...
   private static readonly object _lock = new object();
    ...
    protected virtual void Awake()
    {
        ...
        {
            Debug.Log($"[싱글톤]{typeof(T)} 의 다른 인스턴스가 이미 존재합니다. 이 중복을 파괴합니다.");
            Destroy(gameObject);
        }
    }
    ...
    {
        //객체가 파괴되고 있지만 애플리케이션이 종류 중이 아니라면 문제이기 때문에 로그를 남김
        if(!isQuitting)
        {
